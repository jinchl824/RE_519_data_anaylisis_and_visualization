...
1.  [1 pt] What is the unit of analysis of this study? What is the
    ...
    The unit of analysis in this study is a household-year observation,
    meaning each subsidized household is observed once per year. The
    sample contains about 15.8 million household-year observations
    (2005–2018). Because the same households are followed over multiple
    years, this is a longitudinal dataset rather than a cross-sectional
    one.
...
2.  [0.5 pt] Why do you think U.S. Census Bureau requires us apply for
    ...
    The U.S. Census Bureau restricts access because HUD’s Annual
    Longitudinal Files include sensitive, individual-level information.
    Variables such as household composition, subsidy status, income, and
    geographic identifiers could be used to re-identify households.
    Therefore, researchers must apply through the Federal Statistical
    Research Data Center (FSRDC) system rather than using the data
    publicly like ACS.
...
3.  [0.5 pt] In a research paper, we tend to include two parts of
    ...
    The key explanatory variables are indicators of household
    compositional change, such as a partner, adult, or child entering or
    exiting the household. The response variable is whether the
    household moved during that year (binary: 1 = moved, 0 = not moved).
...
4.  [1 pt] What is the purpose of including local market characteristics
    ...
    Local market characteristics are included so that the model accounts
    for structural differences across housing markets that influence
    mobility. Without these controls, the model may incorrectly
    attribute market-level factors to household-level compositional
    changes. Variables likely to be collinear with local market
    characteristics include rent levels, vacancy rates, poverty rates,
    unemployment rates, and neighborhood income, since these economic
    conditions tend to move together.

5.  [0.5 pt] Can we use normal linear regression for this study?

    A normal linear regression is not appropriate because the outcome is
    binary. A linear probability model would violate assumptions of
    homoscedasticity and could generate predicted values outside the 0–1
    range. Logistic regression is the correct specification.
...
6.  [0.5 pt] The follow picture shows model specification in page 8 of
    ...
    The equation shown on page 8 resembles a linear regression
    specification even though the authors estimate a logistic
    regression. The notation does not show the log-odds transformation
    and also simplifies the structure of the explanatory variables. In
    practice, the model includes multiple household-level and
    market-level controls as well as additional fixed effects. The
    equation is therefore incomplete and potentially misleading.
...
1.  [1 pt] How should we interpret $\beta_1$?
    ...
    In a logistic regression, $\beta_1$ represents the change in the
    log-odds of moving associated with a one-unit increase in X₁,
    holding other variables constant. Equivalently, exp($\beta_1$) gives
...
2.  [0.5 pt] In the result tables of this paper, they use **Odds Ratio**
    ...
    The odds ratio is 1.671. Converting it to the coefficient:\
...
3.  [0.5 pt] How much more likely are households with a compositional
    ...
    An odds ratio of 1.671 means that households experiencing a
    compositional change are about 67% more likely to move than
    households without such changes, controlling for other factors.
...
4.  [1 pt] What are the null and alternative hypotheses being tested by
    ...
    The null hypothesis is that $\beta$ = 0 (compositional change has no
    association with moving).\

    The alternative hypothesis is $\beta$ ≠ 0.\

    The "\*\*\*" symbol indicates statistical significance at p \<
    0.001, meaning very strong evidence against the null hypothesis.

5.  [1 pt] They conducted at least three logistic regression models. Why
    did they do that?

    The authors estimate multiple models to check robustness. Each model
    adds more controls (e.g., demographic characteristics, economic
    variables, market characteristics). If the coefficient for
    compositional change remains stable across models, the result is
    less likely to be driven by omitted-variable bias.
...
6.  [1 pt] The authors only use tables to present results. What is one
    ...
    One way to visualize the odds ratios is a coefficient plot (forest
    plot). Each point represents an odds ratio, with horizontal lines
    showing 95% confidence intervals and a vertical reference line at OR
    = 1. This makes it easier to observe uncertainty and compare effects
    across variables.
...
The term “association” is used because the study is observational.
Without a causal identification strategy, the authors cannot claim that
compositional change causes mobility. They can only document
...
principles. It provides us a framework. It is helpful to know that
machine learning and deep learning are normally run with Python. For the
consistence of this class, we will continue use R. But it is not
difficult to transfer to Python! Some resources:

-   [An Introduction to Python for R
    Users](https://rebeccabarter.com/blog/2023-09-11-from_r_to_python)
-   [Conversion R and
    Python](https://www.mit.edu/~amidi/teaching/data-science-tools/conversion-guide/r-python-data-manipulation/)
...
We will use the same dataset as section A but let's forget the most
assumptions (normality, linearity, etc.) and focus purely on predictive
performance! We will train and evaluate linear regression, ridge
regression, and lasso regression using cross-validation to see which
approach yields the best testing predictions.

### Data Preparation

There are too many variables can be added to the model. Some are not
suitable for a prediction task, such as longitude and latitude. We start
from remove some data columns and merge some of them. After that, do
some cleaning based on inspection.

In a more rigorous machine learning process, we may conduct feature
engineering and selection before training. For example, we could use
[Principal Component Analysis
(PCA)](https://en.wikipedia.org/wiki/Principal_component_analysis) to
reduce dimension or select most important features based on correlation
results. A more comprehensive book on this: [Feature Engineering and
Selection: A Practical Approach for Predictive
Models](http://www.feat.engineering).
...
ridge, we need to normalize features.
...
Please note that The object `rec` is a recipe that has **not been
trained** on data yet (for example, does not normalize).

### Model Specification

We start from specify the lasso regression. In `tidymodels`, the model
type (`linear_reg`) and the computational engine (`glmnet`) are
separated.
...
preprocessing steps) and a model specification.
...
We can define the search space for $\lambda$. `penalty(range = c(a, b))`
...
We show the model performance changes as $\lambda$ increases via a
curve.
...
We can check the final coefficients using `tidy(workflow)`. variables
with estimate = 0 have been removed by lasso regression.
...
We evaluate the performance of the final Lasso model on the held-out
test set. The `predict()` function generates predicted sale prices for
the test data, and `metrics()` computes evaluation measures such as RMSE
and R².
...
the model and explore ridge regression. Reply “yes” once you have
reviewed them.
...
In this section, we will read a recent research paper that applies
several machine learning regression techniques—including Lasso, Ridge,
and tree-based models—to predict housing rental values in three African
countries. The paper, [Predicting the rental value of houses using
hedonic and machine learning regression
models](https://doi.org/10.1371/journal.pone.0244953) examines how
different modeling approaches perform when estimating rental values
based on a wide range of housing characteristics, infrastructure
indicators, and household attributes. You can find the PDF version of
the article in the lab folder. Please skim the paper first and response
to following questions. You can skip the parts beyond regression.

1.  [1 pt] Why do we need regularization in Lasso and Ridge regression?
    What effect does regularization have on variance and bias,
    respectively?

    Regularization is needed because models with many predictors can
    easily overfit the training data. Lasso and Ridge add penalties to
    the size of coefficients, which shrink the model toward simpler
    structures. Regularization reduces variance but increases bias,
    creating a bias–variance tradeoff that usually improves
    out-of-sample performance.

2.  [1 pt] In the model, the variable “roof” equals 1 if the house has a
    mud roof and 0 otherwise. If the Lasso regression sets the
    coefficient of this variable to zero in 2010, can we conclude that
    roof type is not associated with rental value? Why or why not?

    If Lasso assigns a coefficient of zero to the “roof” variable in
    2010, we cannot conclude that roof type has no association with
    rental value. A zero coefficient means that, given the other
    predictors in the model, roof type does not improve predictive
    accuracy. It does not imply the true relationship is zero. The
    result may be driven by multicollinearity or by Lasso’s variable
    selection behavior.

3.  [1 pt] Why do the authors use cross-validation in their analysis?
    And how cross-validation is used to choose the regularization
    parameter ($\lambda$) in Lasso and Ridge regression?

    The authors use cross-validation to estimate how well different
    models generalize to new data. For Lasso and Ridge, cross-validation
    is used to choose the penalty parameter $\lambda$. The algorithm
    fits models across a range of $\lambda$ values, evaluates prediction
    errors in the validation folds, and selects the $\lambda$ that
    minimizes out-of-sample error.

4.  [1 pt] In the prediction performance table, OLS performs best in the
    in-sample evaluation, while Lasso and Ridge outperform OLS in the
    out-of-sample evaluation. Could you guess the reason for this
    pattern?

![Determinants (\~Coefficients) of Housing Rental
Values](assets/ml_results.png)

![In-sample and Out-of-sample Prediction
Performances](assets/prediction_results.png)

OLS performs best in-sample because it fully fits the training data and
has no penalty term. However, this also means it is more likely to
overfit and perform worse on new data. Lasso and Ridge shrink
coefficients, which reduces overfitting and improves out-of-sample
predictions. Therefore, OLS tends to have the lowest training error but
worse test error, while regularized models perform better when
