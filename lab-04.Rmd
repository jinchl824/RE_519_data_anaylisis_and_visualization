...
```{r}
# Theme: Housing Cost Burden among OECD Countries (2024)
...
housing <- data.frame(
  country = c("United States", "Canada", "United Kingdom", 
              "Japan", "Australia", "France", "Germany", "Italy", "OECD average"),
  rent_burden = c(58, 52, 41, 43, 31, 18, 9, 11, 39)
)

housing <- housing %>% 
  mutate(country = reorder(country, rent_burden))

ggplot(housing, aes(x = country, y = rent_burden, fill = rent_burden)) +
  geom_col(width = 0.7, color = "white") +
  geom_text(aes(label = paste0(rent_burden, "%")), 
            hjust = -0.1, size = 4, fontface = "bold", color = "black") +
  scale_fill_gradient(low = "#B3E2CD", high = "#FD996B") +
  coord_flip() +
  annotate("text", x = 1, y = 60, 
           label = "U.S. has the highest rent burden (58%)", 
           color = "#D73027", fontface = "italic", size = 4, hjust = 0) +
  labs(
    title = "Housing Cost Overburden Rate among OECD Countries (2024)",
    subtitle = "Share of low-income private tenants spending more than 40% of their income on rent",
    x = "",
    y = "Share of income spent on rent (%)",
    caption = "Source: OECD Affordable Housing Database (Indicator HC1.2, 2024)"
  ) +
...
# Create a deceptive effect by reversing order and truncating the y-axis
housing <- housing %>% 
  mutate(country = reorder(country, -rent_burden))
...
ggplot(housing, aes(x = country, y = rent_burden, fill = rent_burden)) +
  geom_col(width = 0.7, color = "white") +
  scale_fill_gradient(low = "#B3E2CD", high = "#FD996B") +
  # Truncate y-axis to exaggerate visual differences
  coord_cartesian(ylim = c(30, 60)) +
  labs(
    title = "Perceived Housing Cost Burden (Misleading Example)",
    subtitle = "Share of income spent on rent (truncated scale exaggerates differences)",
    x = "",
    y = "Share of income spent on rent (%)",
    caption = "⚠️ Example of deceptive visualization using OECD data (HC1.2, 2024)"
  ) +
  theme_minimal(base_size = 13) +
  theme(
    plot.title = element_text(face = "bold", size = 16, color = "#D73027"),
    plot.subtitle = element_text(size = 12, color = "gray40"),
...
**How this visualization misleads:**
This version manipulates the viewer’s perception by truncating the y-axis (starting at 30% instead of 0%) and reversing the country order.
As a result, small numerical differences between countries appear visually dramatic, misleading the audience into thinking that European nations have much higher rent burdens than they actually do.

**Design decisions contributing to deception:**

Y-axis truncation exaggerates the gap between countries.

Color gradient reinforces this illusion by visually emphasizing smaller differences.

Reversed order places high-burden countries at the bottom, making the pattern less intuitive to interpret.

**Reflection:**
This exercise demonstrates how easily design choices can distort interpretation, even with accurate data.
A simple scale change or category ordering can mislead the audience without adding or removing any data, showing why visual ethics matter.
