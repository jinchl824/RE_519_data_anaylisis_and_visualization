...
# 1
data(kingco_homes)
kingco_homes_subset <- kingco_homes %>% 
  select(pinx, city, land_val, imp_val, sqft, view_lakewash, view_lakesamm)

#2
kingco_homes_bellevue <- kingco_homes_subset %>% 
filter(
    city =="BELLEVUE"
  )

#3
kingco_homes_bellevue_with_total_val <- kingco_homes_bellevue %>% 
  mutate(
    total_val = land_val + imp_val
  )

#4
kingco_homes_bellevue_with_total_val_desc <- kingco_homes_bellevue_with_total_val%>% 
  arrange(desc(total_val)) 

head(kingco_homes_bellevue_with_total_val_desc,10)

#5
bellevue_mean <-mean(kingco_homes_bellevue_with_total_val$total_val, na.rm = TRUE)
bellevue_var <- var(kingco_homes_bellevue_with_total_val$total_val, na.rm = TRUE)

kingco_homes_bellevue_with_two_lakes_view <- kingco_homes_bellevue_with_total_val %>% 
filter(
    view_lakewash == 1 & view_lakesamm == 1
  )
bellevue_2lake_mean <- mean(kingco_homes_bellevue_with_two_lakes_view$total_val, na.rm = TRUE)
bellevue_2lake_var <- var(kingco_homes_bellevue_with_two_lakes_view$total_val, na.rm = TRUE)

diff_bv_and_2lake_mean <- bellevue_2lake_mean - bellevue_mean
diff_bv_and_2lake_var <- bellevue_var - bellevue_2lake_var

```

------------------------------------------------------------------------

I compared Bellevue homes with and without both lake views. On average, those with both lake views are `r diff_bv_and_2lake_mean` dollars higher in total value. Their variance is `r diff_bv_and_2lake_var` smaller than all Bellevue properties variance, which means prices for dual-lake homes are more stable and concentrated around higher values.
...
group_by(condition) %>% 
  summarise(
    N = n(), 
    avg_total_val_per_sqft = mean((land_val + imp_val)/sqft), 
  ) 
...
set.seed(79)
sample_means <- replicate(
  200,  # we repeat the process (sample -> sample mean -> record) for 200 times
  bellevue_after2020 %>%
...
It’s probably because the sample size is too small, which causes the sample mean to deviate slightly from the true population mean.
...
head(kingco_homes) 
kingco_homes %>%
select(pinx,city,sqft,land_val,imp_val,year_built) %>% 
filter(
    city =="BELLEVUE" 
  ) %>%
  group_by(year_built) %>%
summarise(
    N = n(), # number of rows
    avg_val_per_sqft=mean((land_val+imp_val)/sqft)
  )%>%
arrange(desc(avg_val_per_sqft))
```

> The average property value per square foot was the highest in 1937.
